...
import { CiiAdditionalIdentifier, CiiDto } from 'src/app/models/org';
...
export class ManageOrganisationRegistryConfirmComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      ...
      if (this.id && params.scheme) {
        ...
        this.item$ = this.ciiService.getIdentifiers(ciiOrgId, params.scheme, this.id).pipe(share());
        this.item$.subscribe({
          next: result => {
  ...
  isExistsInCurrentRegistration(identifier: string): boolean {
    const registries: CiiDto[] = JSON.parse(localStorage.getItem('cii_registries') + '');
    if (registries && registries.some((registry) => registry.identifier?.id === identifier) ||
      (registries.some((registry) => registry.additionalIdentifiers &&
